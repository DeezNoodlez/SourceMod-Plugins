...
public Plugin:myinfo =
{
	...
	author = "DeezNoodlez",
	description = "Toggles alltalk at roundend/roundstart; by Munra",
...
public TF2_OnWaitingForPlayersStart()
{
	...
	{
		AllTalkToggle(true);
		PrintToChatAll("\x04Waiting For Players: all talk enabled.");
	}
	else
	{
		g_WaitRound++;
...
{  
   ...
   {
		AllTalkToggle(true);
		PrintToChatAll("\x04All talk enabled.");
   }
}
...
{
	AllTalkToggle(false);
	PrintToChatAll("\x04All talk disabled.");
}
...
public Action:printmessage(Handle:timer)
{
	...
	{
		PrintToChatAll("\x03All talk will turn off in \"%.0f\" seconds.", GetConVarFloat(g_Cvar_smackwarntime));
	}
}
